...
## Ready to paste (updated 2026-08-31 — verified, not assumed)
...
| Video URL | <https://youtu.be/kGiqvn-vrv0> — public, **3:50**, verified 2026-08-31 |
...
- [x] Gemini 3.5+ via Gemini API or Vertex AI — the video visibly names Gemini 3.7 Flash
- [x] Google Agent Framework: **ADK (Python)** — visibly named in the architecture segment
- [x] GCP infra service: **Cloud Run + Firestore** — both consoles are shown in the live-execution segment
...
- [ ] Text description: features, functionality, tech used, data sources (NASA ASRS via HF `elihoole/asrs-aviation-reports`), findings & learnings (they explicitly ask for learnings — include the caught-reward-hack story)
  - [ ] Literal **"The Twist"** section: restraint made mechanical — no-LLM clustering, frozen thresholds the agents cannot touch, locked holdout, citation-stripping critic, and the citation gate we caught validating *shape* instead of *provenance* (a model invented ACNs 1000001–1000005 and the gate kept them). **Corrected 2026-08-29: the originally planned "agent gamed ROUGE and a guard caught it" story never happened** — do not claim it; `eval/runs/` is committed and a judge can check. See `docs/DEVPOST_DRAFT.md` for the paste-ready text. Framing line: "VIGIL's headline feature is the list of things its agents cannot do." (Paste the Twist section from `DEVPOST_DRAFT.md` verbatim; do not retype it.)
...
- [x] Video ≤4 min, public YouTube/Vimeo, English: problem + value prop + live demo + **GCP console proof on screen**; public video verified at **3:50** on 2026-08-31: <https://youtu.be/kGiqvn-vrv0>
