...
|---|---|---|---|
| T1-01 | Severe-but-unclustered queue | ✅ Done | Implemented 2026-08-30 on `main` (commit `a2d719f`): `pipeline.risk.severe_matches`, `pipeline.run_batch.find_severe_singletons`/`run_triage`/`build_artifact_payload`, artifact schema v2 with a legacy-list-compatible loader, and a Severe singletons UI queue. All section 6.3 tests plus a literal section-6.4 acceptance fixture pass (`tests/test_severe_singletons.py`, `tests/test_streamlit_app.py`); full suite 68/68, ruff clean. **Verified live 2026-08-30** (commit `c4334e8`): a real `--live --firestore` run against a local Firestore emulator regenerated `artifacts/demo_run.json` with 1,328 real severe singletons, and a real Chromium browser (Playwright) driven against a live `streamlit run` confirmed the queue renders. Redeploying the Cloud Run UI itself remains the user's own action. See `docs/PHASES.md` Phase 8 for the mirrored status row and full note. |
| T1-02 | ACN evidence drill-down | ✅ Done | Implemented 2026-08-29 on `main` (commit `0660daa`): `agents.critic.extract_cited_acns`, `pipeline.run_batch.build_cluster_evidence` (member + precedent evidence, unresolved-citation failure), `pipeline.store.put_cluster_evidence`, and an evidence selectbox in `ui/streamlit_app.py` shared with the T1-01 singleton panel. All section 7.3 tests pass (`tests/test_evidence.py`, 12 new); full suite 80/80, ruff clean. **Verified live 2026-08-30** (commit `c4334e8`): the same live run produced two real escalated clusters whose live-drafted briefs cite real precedent ACNs outside their own membership (6 and 9 precedent evidence records) -- section 7.4's acceptance criterion, resolved for real rather than via synthetic fixtures alone. See `docs/PHASES.md` Phase 8 for the mirrored status row and full note. |
| T1-03 | Edit-before-approve and required rejection reason | ✅ Done | Implemented 2026-08-30 on `main` (commit `06f32d2`): `pipeline.store.record_approval`/`record_rejection` (shared reason validation, atomic Firestore batch write), an editable brief with a required rejection reason in `ui/streamlit_app.py`, and pure `evaluate_approval`/`build_rejection_value` helpers that re-run the existing citation gate against human edits. Terminal per session; approved download serves `brief_approved` bytes only. All section 8.4 tests pass (`tests/test_store_decisions.py` +9, `tests/test_ui_decisions.py`, 11 new, using Streamlit `AppTest`). **Section-12's literal "manual human-gate smoke" ran for real 2026-08-30**: a real headless Chromium browser (Playwright, not `AppTest`'s bare-mode execution) against a live `streamlit run` process serving the regenerated live artifact -- 7/7 steps passed (uncited edit blocked, valid edit approved and locked, blank reason blocked, real reason rejected and shown), zero console errors. See `docs/PHASES.md` Phase 8 for the mirrored status row and full note. |
| T1-04 | Cross-run hazard identity and history | ✅ Done | Implemented 2026-08-30 on `main` (commit `06f32d2`): `HazardObservation`/`HazardRecord` models, pure `pipeline.store.match_hazard` (Jaccard >0.6, deterministic tiebreak), `record_hazard_observation` on both stores (Firestore's transactional), and a threaded `run_id`/`run_at` through `run_triage`/`build_artifact_payload`. UI renders a cross-run summary, sparkline, and observation table. All section 9.5 tests pass (`tests/test_hazard_history.py`, 15 new). The section-12 mandatory Firestore emulator run was executed for real (`tests/test_firestore_emulator.py`, 6 tests against a live emulator), proving atomic decision writes and cross-instance hazard idempotency -- not just mocked. Full suite 116/116, ruff clean. **Escalated to a genuine live demonstration 2026-08-30** (commit `c4334e8`): ran the real 5,000-report slice through `--live --firestore` twice (two full live agent passes, not a same-process shortcut) against the local emulator; every cluster in the resulting artifact carries `hazard_history` with 2 real chronologically-ordered observations. See `docs/PHASES.md` Phase 8 for the mirrored status row and full note. |
