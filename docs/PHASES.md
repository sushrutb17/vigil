...
| Cloud Run UI service deployed | ✅ Done | **Live: https://vigil-ui-715230861973.us-central1.run.app** (revision `vigil-ui-00002-5fh`, region `us-central1`, `--allow-unauthenticated`). Deployed 2026-08-29 as `vigil-ui-run`. Serves the committed `artifacts/demo_run.json` snapshot; not yet click-tested in the hosted environment. |
| ⚠️ Regenerate `artifacts/demo_run.json` before recording | ⬜ Not Started | **The deployed UI serves this file**, and it was generated *before* the citation-format fix (`f2fe88a`), so the hosted briefs still show empty `## Precedent` / `## Risk Assessment` sections and the stray `# Cleaned Brief` heading. Run `make run-live` then `make artifact` locally (real 5k slice, needs credentials + ~23 Analyst calls), commit the snapshot, redeploy `vigil-ui`. Until then the live URL under-sells the system. **Do this after the empty-section fix above** so the regenerated snapshot picks up both fixes in one pass — regenerating twice wastes ~23 Analyst calls. Note the real slice *will* produce genuine Precedent content (unlike the 6-report fixture, it has non-member reports sharing a component), so this is also the run that first exercises Precedent end to end. |
| UI Approve/Reject verified against hosted Firestore | ⬜ Not Started | Unit-tested (`tests/test_store_decisions.py`) and IAM is in place (`vigil-ui-run` holds `roles/datastore.user`, `GOOGLE_CLOUD_PROJECT` is set on the service), but no one has clicked the buttons on the deployed URL and confirmed a `clusters/` status change and a `rejections/` document appear. Cheap to verify, and it is the human-gate story the whole design rests on. |
| Cloud Run batch job deployed | ✅ Done | Job `vigil-batch` deployed 2026-08-29 in `us-central1` as `vigil-batch-run`, key mounted from Secret Manager. Runs `--demo --live --firestore`, so one execution exercises all three mandatory stack components (ADK/Gemini, Cloud Run, Firestore) and creates the Firestore collections. **Executed successfully** — `vigil-batch-96p89`, `-w952b`, `-9kjgs`, `-29f4n`, `-dwpfp`. `-dwpfp` is the first execution built from a `jobs deploy` carrying `f2fe88a`, and its Firestore brief confirms the fix landed. Note for future sessions: `jobs execute` reruns the image already built; a code change needs `jobs deploy` first (cost us two wasted executions). |
