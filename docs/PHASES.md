...
| Feature | Status | Note |
|---|---|---|
| `agents/definitions.py` — `build_agent_graph`: Extractor + Dedup (`SequentialAgent`), Analyst, Coordinator (`ParallelAgent`: Precedent ∥ Risk ∥ Brief Writer), Critic | ✅ Done | The graph object builds all seven agents plus the `intake`/`coordinator` wrappers. **Of these, six have executed against a live model** — Analyst, Precedent, Risk, Brief Writer and Critic in the batch path (`agent_log` in Firestore is the receipt), and Extractor in the offline eval. **Dedup has never run and the `intake` `SequentialAgent` is never invoked**: ASRS pre-merges Report 1/Report 2 into one row per ACN, so there is no runtime dedup work. The wrappers are retained because they document the intended Sequential/Parallel shape, but `run_batch.py` drives the individual agents directly and the stage-4 fan-out is plain-Python, not the `ParallelAgent`. |
...
| Cluster purity/ARI vs `Events_Anomaly` on real data | ✅ Done | Run 2026-08-29 on the real 5,000-report slice via `make eval-offline` (`eval/offline_report.py`, deterministic, no model calls). **The results are poor and are being reported rather than tuned away.** Purity **0.301** against a majority-class baseline of **0.219** — a real but modest +0.08. Adjusted Rand **0.0018**, i.e. the clusters essentially do not recover NASA's anomaly partition. Noise fraction **0.837**: 23 clusters covering 816 of 4,998 labelled reports, leaving 84% unclustered. Partial defence, offered as interpretation and not as an excuse: `Events_Anomaly` is a coarse 58-value administrative taxonomy whose largest bucket ("ATC Issue All Types", 1,097 reports) spans operationally unrelated events, so a cluster can be operationally coherent and taxonomically mixed — but ARI 0.0018 is too low to explain away entirely. **See the guard breach row below.** |
| ⚠ Noise fraction breaches the project's own declared guard | 🚫 Blocked | **Found 2026-08-29 by finally running the metric.** EVAL.md predeclares `noise_fraction < 0.40` as a reward-hacking tripwire and `eval/guards.evaluate_guards` implements it — but that function is only ever invoked on the extractor promotion loop, so **nothing had ever checked it against the clustering stage it was written for.** Measured value is **0.837**, more than double the limit. `eval/offline_report.py` now reports `noise_fraction_guard_passed: 0.0` explicitly rather than leaving a reader to compute it. **Deliberately NOT fixed under deadline:** tuning `min_cluster_size`/`min_samples` until the noise number falls below its own guard, hours before submission and with no held-out check on the clustering stage, is precisely the reward-hacking this project claims to have engineered against. The honest options are (a) raise `min_cluster_size` and re-measure properly, or (b) argue the guard's threshold was wrong for a hazard-detection system where most reports genuinely are one-offs — but that argument has to be made *before* seeing whether it helps, not after. Left as a documented, measured failure. |
| Critic eval — seeded uncited-claims catch rate | ✅ Done | Run 2026-08-29 via `make eval-offline`. **400 seeded claims across 200 trials, catch rate 1.000** for both planted failure modes — a claim with no citation, and a claim citing a syntactically valid ACN that exists in no report. Crucially the harness also plants a *correctly cited* claim per trial and measures retention (**1.000**), because a gate that simply deleted everything would otherwise score a perfect catch rate. This is the one predeclared metric that came back clean. |
...
| Parallel fan-out partial-failure handling (2-of-3 sub-agents survive → `DEGRADED` brief) | ✅ Done | **Demonstrated live 2026-08-29**: `python -m pipeline.run_batch --demo --live --fail-agent risk` produced a `DEGRADED` brief whose `## Risk Assessment` fell back to the cited deterministic line while the Brief Writer's model-authored section survived, every claim still ACN-cited. `--fail-agent {precedent,risk,brief_writer,critic}` (repeatable, requires `--live`) raises a real `InjectedFailure` at the call site rather than fabricating a `None`, so the demo travels the exact path a genuine API failure takes — the same `_call_or_none` and the same failure accounting. It prints a loud stderr banner because an injected run must never be mistaken later for real output, and it is *not* an auto-approve-style convenience flag (guardrail #6 untouched: the human gate is unaffected). |
| `DEGRADED` banner cannot be dropped by the Critic | ✅ Done | Found 2026-08-29 while writing the failure-tolerance tests. The Critic's response is used verbatim as the brief, so the banner survived only if the model chose to echo it — and `CRITIC_INSTRUCTION` asks it to preserve *headings*, which `DEGRADED` is not. A reviewer had no reliable way to tell a partial-failure brief from a clean one. Whether a sub-agent failed is a fact the orchestrator already knows, so `_reassert_degraded` now re-inserts the banner after the gate if it went missing — the same move as `_backfill_empty_sections`: the LLM writes prose, but never stands alone between a reviewer and a safety-relevant marker. Locked by a test whose fake critic deliberately strips the banner. |
...
| UI: "NEW THIS RUN" badge on clusters | ✅ Done | Implemented 2026-08-29. `ClusterAssessment.newly_escalated` is set in `run_batch` from the escalation ledger's existing member-set overlap check, flows into the artifact for free via `asdict`, and renders as a 🆕 marker in the sidebar queue plus a banner in the detail pane. Deliberately a **new boolean rather than reusing the stored `status` string**, whose `"new"` value confusingly means "not escalated *this* run" and which the UI and tests already depend on — renaming that remains a separate change. Locked by a test that runs the same batch twice and asserts the flag is true on first sight and false on the repeat. **Honest limitation:** in the committed artifact every escalated cluster is new, because the snapshot is one fresh run against an empty ledger; the badge only discriminates when running against persistent Firestore across weeks, which is what the Cloud Scheduler trigger produces. |
| Citation list capped in briefs | ✅ Done | Fixed 2026-08-29. The real 5k slice contains a 629-member maintenance megacluster whose deterministic brief spelled out every member ACN twice — an **18,068-character** wall of citations that no human would read and that a judge would have seen in the deployed UI. `agents.critic.format_citations` caps the inline list at 12 and states the remainder (`(+617 more in this cluster)`) rather than dropping it silently; the UI already shows the full member list beside the brief. Guardrail #4 is untouched — it requires every claim to cite *an* ACN, not every ACN, and the surviving citations still have to be real reports, which a test asserts. Largest brief on the real slice is now **636 characters**. |
...
| README: spin-up steps, architecture PNG, metrics table | ✅ Done | Spin-up steps exist. **Metrics table added 2026-08-29** with the real Phase 5 numbers (baseline vs v1 vs promoted v2, dev and holdout side by side per EVAL.md), plus a `make improve` section, a failure-tolerance section with the exact `--fail-agent` command, and the loop's guardrails written up as enforced-by-test rather than promised. **Architecture PNG done 2026-08-29**: the mermaid source was rewritten first, because it had drifted into three false claims (risk shown as a product rather than a deterministic weighted sum, Extractor/Dedup shown in the operational batch path after they were scoped out, and the holdout shown running *before* the guards). Rendered via mermaid-cli into `docs/architecture.png` plus a second figure `docs/self-improvement-loop.png`, both embedded in the README. |
