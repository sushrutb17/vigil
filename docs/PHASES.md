...
| Cloud Run batch job deployed | ✅ Done | Job `vigil-batch` deployed 2026-08-29 in `us-central1` as `vigil-batch-run`, key mounted from Secret Manager. Runs `--demo --live --firestore`, so one execution exercises all three mandatory stack components (ADK/Gemini, Cloud Run, Firestore) and creates the Firestore collections. Deployed but **not yet executed** — see the row below. |
