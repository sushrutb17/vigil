...
| Cloud Run batch job deployed | ✅ Done | Job `vigil-batch` deployed 2026-08-29 in `us-central1` as `vigil-batch-run`, key mounted from Secret Manager. Runs `--demo --live --firestore`, so one execution exercises all three mandatory stack components (ADK/Gemini, Cloud Run, Firestore) and creates the Firestore collections. **Executed successfully** — `vigil-batch-96p89`, `-w952b`, `-9kjgs`, `-29f4n`. Note for future sessions: `jobs execute` reruns the image already built; a code change needs `jobs deploy` first (cost us two wasted executions). |
